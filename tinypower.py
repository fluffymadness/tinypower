...
class SystemTrayIcon(QtGui.QSystemTrayIcon):	
    ...
    def __init__(self, parent=None):
        self.batterychecker = BatteryChecker(self)
        self.connect(self.batterychecker, self.batterychecker.setIcon,self.set_icon)
...
class BatteryChecker(QtCore.QThread):
    ...
    def __init__(self, uiObject):
        ...
        self.setIcon = QtCore.SIGNAL("setIcon")

    def run(self):
        while(self.running == 1):
            info = self.get_battery_info()
            icon_name = self.get_icon_name(info['state'],info['percentage'])
            self.emit(self.setIcon, icon_name, info['tooltip'])
            time.sleep(2)    
